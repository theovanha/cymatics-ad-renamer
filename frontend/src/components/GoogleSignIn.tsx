import { api } from '../api/client';
import type { AuthStatus } from '../types';

interface GoogleSignInProps {
  authStatus: AuthStatus | null;
  onAuthChange: () => void;
}

export default function GoogleSignIn({ authStatus, onAuthChange }: GoogleSignInProps) {
  const handleSignIn = () => {
    api.login();
  };

  const handleSignOut = async () => {
    await api.logout();
    onAuthChange();
  };

  if (!authStatus) {
    return (
      <div className="google-signin loading">
        <span className="spinner-sm"></span>
        Loading...
      </div>
    );
  }

  if (authStatus.authenticated && authStatus.user) {
    return (
      <div className="google-signin authenticated">
        <div className="user-info">
          {authStatus.user.picture && (
            <img 
              src={authStatus.user.picture} 
              alt={authStatus.user.name}
              className="user-avatar"
            />
          )}
          <div className="user-details">
            <span className="user-name">{authStatus.user.name}</span>
            <span className="user-email">{authStatus.user.email}</span>
          </div>
        </div>
        <button onClick={handleSignOut} className="btn-secondary btn-sm">
          Sign Out
        </button>

        <style>{`
          .google-signin {
            display: flex;
            align-items: center;
            gap: 1rem;
            padding: 0.75rem 1rem;
            background: var(--bg-secondary, #1e1e2e);
            border-radius: 8px;
            margin-bottom: 1.5rem;
          }

          .google-signin.loading {
            justify-content: center;
            color: var(--text-secondary, #888);
          }

          .user-info {
            display: flex;
            align-items: center;
            gap: 0.75rem;
            flex: 1;
          }

          .user-avatar {
            width: 40px;
            height: 40px;
            border-radius: 50%;
            border: 2px solid var(--accent-color, #6366f1);
          }

          .user-details {
            display: flex;
            flex-direction: column;
          }

          .user-name {
            font-weight: 600;
            color: var(--text-primary, #fff);
          }

          .user-email {
            font-size: 0.8rem;
            color: var(--text-secondary, #888);
          }

          .btn-sm {
            padding: 0.4rem 0.75rem;
            font-size: 0.8rem;
          }
        `}</style>
      </div>
    );
  }

  return (
    <div className="google-signin not-authenticated">
      <div className="signin-prompt">
        <span className="signin-icon">🔒</span>
        <span>Sign in with Google to access Drive folders</span>
      </div>
      <button onClick={handleSignIn} className="btn-google">
        <svg width="18" height="18" viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        Sign in with Google
      </button>

      <style>{`
        .google-signin {
          display: flex;
          align-items: center;
          gap: 1rem;
          padding: 0.75rem 1rem;
          background: var(--bg-secondary, #1e1e2e);
          border-radius: 8px;
          margin-bottom: 1.5rem;
        }

        .signin-prompt {
          display: flex;
          align-items: center;
          gap: 0.5rem;
          flex: 1;
          color: var(--text-secondary, #888);
        }

        .signin-icon {
          font-size: 1.2rem;
        }

        .btn-google {
          display: flex;
          align-items: center;
          gap: 0.5rem;
          padding: 0.5rem 1rem;
          background: white;
          color: #333;
          border: none;
          border-radius: 4px;
          font-weight: 500;
          cursor: pointer;
          transition: box-shadow 0.2s;
        }

        .btn-google:hover {
          box-shadow: 0 2px 8px rgba(0, 0, 0, 0.3);
        }
      `}</style>
    </div>
  );
}
